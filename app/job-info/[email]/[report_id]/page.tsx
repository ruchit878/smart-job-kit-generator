    ...
    fetch(`${API_URL}user-dashboard?user_email=${email}&report_id=${report_id}`)
      ...
      .then((data) => {
        if (data?.report) {
          ...
          localStorage.setItem('report_id', data.report.id || data.report_id || report_id)
...
if (isLoading || loading || !jobData) {
  // Show spinner until both auth and data are loaded
  return (
...
async function handleGenerateResume() {
  ...
  // Build a payload array for the API, marking in_job: true
  ...
  // Pass as JSON string if API expects a list of objects, else just names:
  form.append('additional_skills', selectedSkills.join(', ')) // instead of JSON.stringify(payloadSkills)
...
const downloadCoverLetterDocx = async () => {
  ...
  // If resume and cover letter exist, show them as before:
  ...
  // If resume/cover missing, show Continue/Skills flow:
  const gaps = safeJsonArray(jobData.gaps)
                      ...
                      return (
                        <tr key={gapSkill} className="even:bg-gray-50">
                          ...
                          <td className="px-3 py-2 text-center">
                            <span className="text-green-600">✔</span> {/* Always in job! */}
                          </td>
                          <td className="px-3 py-2 text-center">
                            {skillObj?.in_resume ? (
